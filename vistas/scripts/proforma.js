...
function showDetails(){
    ...
    cards.forEach(card=>{
        card.querySelector('img').addEventListener('click', ()=>{
            modal.children[0].innerHTML = card.children[1].children[0].innerHTML;
       ...
       if( i > inicio && i <= final){
        ...
        for(let valor in filterProformas[proforma]){
            ...
            if(valor == 'id_prof'){
                ...
                td.innerText = 'SMS23-'+filterProformas[proforma][valor];
                tr.appendChild(td);
...
function fillFormProfR(){
    ...
    document.getElementsByName('tpo_valido_profR')[0].value = '15 dias';
    document.getElementsByName('cond_pago_profR')[0].innerHTML = `A CONTRA ENTREGA
DATOS PARA REALIZAR DEPÓSITOS:
A nombre de: SMS INTEGRACIÓN Y CONTROL LTDA.
Banco: BISA
No. de cuenta: 1675590016`;
    document.getElementsByName('tpo_entrega_profR')[0].innerHTML = `Todo Previa confirmación y orden de compra.`;
...
let mdfPproforma = {};
function readMdfProforma(tr){
    tablemProfMW.classList.add('modal__show');
    let id_prof = tr.children[1].innerText.slice(6);
    let formData = new FormData();
    formData.append('read_mdf_Proforma', id_prof)
    fetch('../controladores/proforma.php', {
        method: "POST",
        body: formData
    }).then(response => response.json()).then(data => {
        ...
        let i=1;
        tbody.innerHTML = '';
        for(let proforma in data){
            let tr = document.createElement('tr');
            for(let dato in data[proforma]){
                let td = document.createElement('td');
                if(dato == 'id_mprof'){
                    td.innerText = i;
                    tr.appendChild(td);
                    td = document.createElement('td');
                    td.innerText = data[proforma]['id_mprof'];
                    td.setAttribute('hidden', '');
                    tr.appendChild(td);
                    td = document.createElement('td');
                    td.innerText = data[proforma]['num_proforma_mprof'];
                    td.setAttribute('hidden', '');
                    tr.appendChild(td);
                    td = document.createElement('td');
                    td.innerText = 'SMS23-'+data[proforma]['num_proforma_mprof']+'-'+i;
                    tr.appendChild(td);
                    i++;
                }else if(dato == 'num_proforma_mprof'){
                }else if(dato == 'fecha_mprof'){
                    td.innerText = data[proforma][dato].slice(0,10);
                    tr.appendChild(td);
                }
            ...
            let td = document.createElement('td');
            td.innerHTML = `
            <img src='../imagenes/pdf.svg' onclick='pdfProforma(this.parentNode.parentNode, "mprof")'>
...
//---------------------------------------------------------------GENERAR PDF-----------------------------------------------------
//-------Pide la informacion de la proforma
function pdfProforma(tr, clave){
    if(clave == 'prof'){
        let id_prof = tr.children[1].innerText.slice(6);
        for(let proforma in filterProformas){
            if(filterProformas[proforma]['id_prof']==id_prof){
                imprimirProforma(filterProformas[proforma], clave);
            }
        }
    }else{
        let id_mprof = tr.children[1].innerText;
        for(let proforma in mdfPproforma){
            if(mdfPproforma[proforma]['id_mprof']==id_mprof){
                imprimirProforma(mdfPproforma[proforma], clave);     
            }
        }
    }   
}
//-------Crear el pdf de la proforma
function imprimirProforma(object, clave){
    let proforma = JSON.stringify(object);
    let formData = new FormData();
...
function readProf_prod(){
    ...
    let formData = new FormData();
    formData.append('readProf_prod', id_prof)
    fetch('../controladores/proforma.php', {
        method: "POST",
        body: formData
    }).then(response => response.json()).then(data => {
        for (let clave in data){
            cartProduct_pfpd(data[clave]);
        }
    }).catch(err => console.log(err));
}
...
function cartProduct_pfpd(product){
    ...
    let html = 
        ...
        <img src="../imagenes/trash.svg" onClick="remove_pfpd(this.parentNode)">
...
function openPreviwProducts(){
    ...
    productos.forEach(producto=>{
        html += `<tr>
                    ...
                    <td>${producto.children[4].value} <b style='color: #31b150;'>${moneda}</b></td>
                    <td>${producto.children[5].value} <b style='color: #31b150;'>${moneda}</b></td>
                </tr>`
        i++;
    });
...
const formNotaEntregaR = document.getElementById('formNotaEntregaR');
formNotaEntregaR.addEventListener('submit', createNotaEntrega);
function createNotaEntrega(){
    //-----Cambiar el estado de la proforma
    if (confirm('¿Esta usted seguro?')){
        event.preventDefault();
        let formData = new FormData(formNotaEntregaR);
...
function searchInventoriesMW(){
    filterInventoriesMW = {};
    for(let inventory in inventoriesMW){
        for(let valor in inventoriesMW[inventory]){
            if(selectSearchInvMW.value == 'todas'){
                if(valor == 'codigo_prod' ||  valor == 'nombre_prod' || valor == 'descripcion_prod'  || valor == 'cost_uni_inv'){
                    if(inventoriesMW[inventory][valor].toLowerCase().indexOf(inputSearchInvMW.value.toLowerCase())>=0){
                        filterInventoriesMW[inventory] = inventoriesMW[inventory];
                        break;
                    }
                }
            }else{
