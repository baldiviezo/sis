...
function tableArmed(page) {
    ...
    const armados = filterArmeds.slice(inicio, final);
    tbodyCustomer.innerHTML = '';
    armados.forEach((armed, index) => {
        const tr = document.createElement('tr');
        tr.setAttribute('id_rmd', armed.id_rmd);
        ...
        tdAlmacen.innerText = armed.almacen_rmd == 0 ? 'El Alto' : 'Av. Arce';
        tr.appendChild(tdAlmacen);

        const tdNumero = document.createElement('td');
        tdNumero.innerText = armed.numero_rmd;
        tr.appendChild(tdNumero);
        const tdfecha = document.createElement('td');
        tdfecha.innerText = armed.fecha_rmd;
        ...
        const tdProforma = document.createElement('td');
        tdProforma.innerText = armed.proforma_rmd;
        tr.appendChild(tdProforma);

        const tdCodigo = document.createElement('td');
        tdCodigo.innerText = armed.codigo_rdpd;
        tr.appendChild(tdCodigo);

        const tdCantidad = document.createElement('td');
        tdCantidad.innerText = armed.cantidad_rdpd;
        ...
        tdObservacion.innerText = armed.observacion_rmd;
    ...
    if (columnOne.querySelectorAll('.cart-item').length > 0 && columnTwo.querySelectorAll('.cart-item').length > 0) {
        ...
        if (count == true) {
            ...
            columnOne.querySelectorAll('.cart-item').forEach(item => {
                let valor = {};
                valor['fk_id_prod_rdpd'] = item.getAttribute('id_prod');
                valor['codigo_rdpd'] = item.children[2].innerText;
                valor['cantidad_rdpd'] = item.children[3].value;
                valor['estado_rdpd'] = 0;
                array.push(valor);
            })
            ...
            if (confirm('¿Esta usted seguro?')) {
                if (rqstArmed == false) {
                    ...
                    formData.set("almacen_rmdR", almacen_rmdR.value);
