...
foreach ($pf_pd as $producto) {
    $costoTotal =  $producto['cantidad_ocpd']*$producto['cost_uni_ocpd'];
    $fpdf->Row(array($i, utf8_decode($producto['codigo_prod']),utf8_decode($producto['descripcion_prod']), $producto['cantidad_ocpd'], number_format($producto['cost_uni_ocpd'], 2, ',', '.'), number_format($costoTotal, 2, ',', '.')), null);
    $i++;
